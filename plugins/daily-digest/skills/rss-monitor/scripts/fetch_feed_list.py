...
Wechat-specific step — run before check_updates.py when the feed list is stale.
"""
...
def fetch_url(url, timeout=30):
    ...
    Uses the shared _common helpers (SSL context + decode) but with a simple
    string-return contract (this script doesn't need ETag caching — the feed
    list is fetched at most weekly).
    """
...
def output_age_valid(output_path):
    """Check the actual feeds_wechat.json freshness from its own metadata.
...
def main():
    ...
    parser.add_argument("--cache", default=None,
                        help="Cache metadata file path (legacy; output freshness is read from feeds_wechat.json itself)")
    ...
    # Check cache validity. Primary source of truth is feeds_wechat.json's own
    # metadata.fetch_time; the sidecar --cache file is kept for backward compatibility.
    ...
    print(f"\nFeed list updated: {meta['total_count']} feeds "
          f"({meta['active_count']} active, {meta['discontinued_count']} discontinued)")
