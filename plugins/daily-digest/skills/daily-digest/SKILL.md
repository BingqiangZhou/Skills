...
daily-digest (this skill — orchestrate + summarize + report)
  │
  ├─ rss-monitor          → collects → workspaces/daily-digests/data/rss/latest_updates.json
  ├─ github-monitor       → collects → workspaces/daily-digests/data/github-monitor/latest_updates.json
  └─ tool-update-monitor  → collects → workspaces/daily-digests/data/tool-update-monitor/latest_updates.json
```
...
{project_root}/                    # The user's current project (cwd at run time)
  workspaces/daily-digests/        # Whole pipeline: middleware + reports
    data/                          # Collector + batch intermediates
      ...
      rss/  github-monitor/  tool-update-monitor/   # collectors' latest_updates.json
...
```bash
mkdir -p "{project_root}/workspaces/daily-digests/data/daily-digest"
...
workspace under `{project_root}/workspaces/daily-digests/data/<source>/latest_updates.json`. If a
...
cd "{skill_directory}/../rss-monitor" && python scripts/fetch_articles.py \
  -i "{project_root}/workspaces/daily-digests/data/rss/latest_updates.json" \
  -o "{project_root}/workspaces/daily-digests/data/rss/latest_updates.json" --delay 2.0
```

See the rss-monitor SKILL.md for full flag details. After this step, read
`workspaces/daily-digests/data/rss/latest_updates.json` and check `metadata.update_count`.
...
`workspaces/daily-digests/data/github-monitor/latest_updates.json` and check
...
cd "{skill_directory}" && python scripts/prepare_batches.py \
  --source rss \
  --input "{project_root}/workspaces/daily-digests/data/rss/latest_updates.json" \
...
cd "{skill_directory}/../rss-monitor" && python scripts/merge_summaries.py \
  --batch-dir "{project_root}/workspaces/daily-digests/data/daily-digest" \
  --pattern "rss_ai_summaries_batch_*.json" \
  -o "{project_root}/workspaces/daily-digests/data/daily-digest/rss_ai_summaries.json"
```
...
cd "{skill_directory}" && python scripts/prepare_batches.py \
  --source github \
  --input "{project_root}/workspaces/daily-digests/data/github-monitor/latest_updates.json" \
  --output-dir "{project_root}/workspaces/daily-digests/data/daily-digest" \
  --prefix "github"
```
...
cd "{skill_directory}/../github-monitor" && python scripts/merge_summaries.py \
  ...
  -o "{project_root}/workspaces/daily-digests/data/daily-digest/github_ai_summaries.json"
```
...
cd "{skill_directory}" && python scripts/generate_unified_report.py \
  ...
  --digest-narrative "{project_root}/workspaces/daily-digests/data/daily-digest/digest_narrative.json" \
  --github-input "{project_root}/workspaces/daily-digests/data/github-monitor/latest_updates.json" \
  --github-summaries "{project_root}/workspaces/daily-digests/data/daily-digest/github_ai_summaries.json" \
  --tool-input "{project_root}/workspaces/daily-digests/data/tool-update-monitor/latest_updates.json" \
  --tool-highlights "{project_root}/workspaces/daily-digests/data/daily-digest/tool_ai_highlights.json" \
